...
class Solution {
public:
    ll caldist(vector<int> from, vector<int> to){
    ...
    int minDistance(int height, int width, vector<int>& tree, vector<int>& squirrel, vector<vector<int> >& nuts) {
        ...
        rep(i, nuts.size()){
            ll tdist = caldist(nuts[i], tree);
            ll sdist = caldist(nuts[i], squirrel);
            dist.pb(mp(tdist, sdist));

            numtsdist += (tdist * 2);
            ans = min(ans, sdist - tdist);
        }

        return ans + numtsdist;
    }
};
