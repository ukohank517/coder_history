...
public:
    void gameOfLife(vector<vector<int> >& board) {
                ...
                if(board[i][j]%10 == 1){
                    for(ll a = -1; a <= 1; a ++){
                        for(ll b = -1; b <= 1; b ++){
                            ...
                            ll ni = i + a;
                            ll nj = j + b;
                            if(ni < 0 or ni >= board.size()) continue;
                            if(nj < 0 or nj >= board[i].size()) continue;
                            board[ni][nj] += 10;
                        }
                    }
                }
        ...
        rep(i, board.size()){
            rep(j, board[i].size()){
                ll now_state = board[i][j] % 10;
                ll neighbor = board[i][j] / 10;
                if(now_state == 1 and neighbor == 2) board[i][j] = 1;
                else if(now_state == 1 and neighbor == 3) board[i][j] = 1;
                else if(now_state == 0 and neighbor == 3) board[i][j] = 1;
                else board[i][j] = 0;
                // if(now_state == 1){
                //     if(neighbor != 2 and neighbor != 3) board[i][j] = 0;
                //     else board[i][j] = 1;
                // }else{
